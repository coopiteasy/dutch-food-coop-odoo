...
class CwaProductImportCblcode(models.TransientModel):
    ...
    cblcode_ids = fields.One2many(
        'cwa.product.import.cblcode.wizard', 'wizard_id',
        string='CBL Code', default=_default_cblcode_ids)
...
class CwaProductImportCblcodeWizard(models.TransientModel):
    ...
    wizard_id = fields.Many2one('cwa.product.import.cblcode', string='Wizard',
        required=True, ondelete='cascade')
    trans_id = fields.Many2one('cwa.product.cblcode')
    cwa_product_id = fields.Many2one(
        'cwa.product', required=True, ondelete='cascade')
    source_value = fields.Char('CBL Code')
    internal_category = fields.Many2one(
        'product.category', 'Internal category')
    pos_category = fields.Many2one(
