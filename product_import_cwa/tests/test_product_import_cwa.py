...
class TestProductImportCwa(TransactionCase):
    ...
    def translate_cblcode(self, prod):
        ...
        wizard.cblcode_ids.write(dict(
            pos_category=pos_categ.id,
            internal_category=categ.id
        ))
        wizard.action_apply()
    ...
    def test_product_import_cwa(self):
        ...
        # find "Boekweit"
        cwa_prod1 = cwa_product_obj.search([
            ('omschrijving', '=', 'BOEKWEIT')
        ])
        self.assertEquals(len(cwa_prod1), 1)
        ...
        count = cwa_product_obj \
            .with_context(new_cursor=False) \
            .import_xml_products(file2)
        self.assertEquals(count, 1)
        count = cwa_product_obj.search_count([])
        self.assertEquals(count, 65)
        cwa_prod2 = cwa_product_obj.search([
            ('omschrijving', '=', 'BOEKWEIT')
        ])
        self.assertEquals(len(cwa_prod2), 1)
        ...
        self.assertEquals(self.env['cwa.product.brands'].search_count([]), 1)
        self.assertEquals(self.env['cwa.product.uom'].search_count([]), 1)
        self.assertEquals(self.env['cwa.product.cblcode'].search_count([]), 1)
        self.assertEquals(self.env['cwa.vat.tax'].search_count([]), 1)

        # Import the Boekweit product into the system,
        # both product and supplier details
        cwa_prod2.to_product()
        self.assertEquals(cwa_prod2.state, 'imported')

        # test if product.supplier record correctly loaded
        supplierinfo_obj = self.env['product.supplierinfo']
        supp_info1 = supplierinfo_obj.search([
            ('product_name', '=', 'BOEKWEIT')
        ])
        self.assertEquals(len(supp_info1), 1)
        ...
        self.assertEquals(supp_info1.unique_id, '1007-1001')
        # test if ingangsdatum is correctly loaded (2016-02-11)
        _date = datetime.date(2016, 2, 11)
        self.assertEquals(supp_info1.ingangsdatum, _date)
        self.assertEquals(supp_info1.date_start, _date)
        ...
        cwa_dup1 = cwa_product_obj.search([
            ('eancode', '=', '8711812421205')
        ])

        # Should have separate supplier numbers
        self.assertNotEqual(cwa_dup1[0].leveranciernummer,
                            cwa_dup1[1].leveranciernummer)
        ...
        self.assertEquals(len(prod1.seller_ids), 2)
